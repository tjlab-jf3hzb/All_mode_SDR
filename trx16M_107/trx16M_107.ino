...
ver. 1.04  Sep. 25, 2024  Modified si5351.cpp
ver. 1.05  Sep. 26, 2024  Modified frequency shift method at CW RX mode
ver. 1.06  Oct.  4, 2024  Modified display color
...
void setup(void) {
  ...
  co_side_tone = (float *)ps_malloc( (fsample/10)*sizeof(float) );

  float dt = 2.0f*M_PI*10.0f/(float)fsample; 
  for(int i=0; i<(fsample/10); i++) 
  {
    co_side_tone[i]=cos( (float)i*dt );
    side_tone[i]=sin( (float)i*dt );
  }
...
void loop(void) {
  ...
  if (rxfb == ESP_OK && readsize==BLOCK_SAMPLES*2*4) 
  {
    ...
    if(f_TX == false)
    {
      ...
      if(f_MODE==CWU)
      {
        float Ilo, Qlo, Ist, Qst;
        for(int i=0; i<BLOCK_SAMPLES; i++)
        {
          Ilo = LO_I[pt_LO];
          Qlo = LO_Q[pt_LO];
          Ist = co_side_tone[pt_st];
          Qst = side_tone[pt_st];
          I_RXsig[i] = 10.0f*Lch_in[i] * (Ilo*Ist - Qlo*Qst);
          Q_RXsig[i] = 10.0f*Lch_in[i] * (Qlo*Ist + Ilo*Qst);
          pt_LO++; if(pt_LO==N_LO) pt_LO=0;
          pt_st+= fo_CW/10; if(pt_st >= fsample/10) pt_st -= fsample/10;
        }
      }
      else if(f_MODE==CWL)
      {
    ...
    else
    {
      ...
      if( (f_MODE == USB) || (f_MODE == LSB) )
      {
        ...
        if(f_MODE == LSB)
        {
          for(int i=0; i<BLOCK_SAMPLES; i++){
            Rch_out[i] = 0;
            Lch_out[i] = ( Iu_TXsig[i]*LO_Q[pt_LO] + Qu_TXsig[i]*LO_I[pt_LO] ) * OUTLEVEL_SSB; //Apr. 16, 2025
            pt_LO++; if(pt_LO==N_LO) pt_LO=0;
          }
        }
        else  //USB
