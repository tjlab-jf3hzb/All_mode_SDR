...
class FRAME {
  private:
  public:
  float xo=0, yo=0, xw=106, yw=67;
  int frm=5;// 枠全体の太さ
  int vzl=3;//  < frm

  uint32_t up_color = 0x404040U;
  uint32_t btm_color = 0xB0B0B0U;
  uint32_t left_color = 0x505050U;
  uint32_t right_color = 0x606060U;
  uint32_t vz_color = 0x808080U;

  void disp(int yoff){
    for(int i=0; i<vzl; i++)
      sprites[flip].drawRect(xo+i, yo+i -yoff, xw-2*i, yw-2*i,vz_color); 
    for(int i=vzl; i<frm; i++)
    {
      sprites[flip].drawFastHLine(xo+i, yo+i -yoff, xw-2*i, up_color);  
      sprites[flip].drawFastHLine(xo+1+i, yo+yw-i -yoff, xw-2*i, btm_color); 
      sprites[flip].drawFastVLine(xo+i, yo+1+i -yoff, yw-2*i, left_color); 
      sprites[flip].drawFastVLine(xo+xw-i, yo+i -yoff, yw-2*i, right_color);
    }
  }
