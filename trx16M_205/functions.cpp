...
void METER::draw(float value, int yoff)
{
  ...
  // 針のはみ出た部分を消す
  sprites[flip].fillRect(xposition, yposition+imgHeight-1 -yoff, imgWidth, imgHeight-yposition, BGCol);
}
...
void GRAPH::plot(float *xdat, float *ydat, int N, uint32_t cl, int yoff)
{
  ...
  for(int i=0; i<N-1; i++)
  {
    ...
    if(  (xs>=x_min) && (xe<=x_max) ) 
    {
      ...
      xs = width*(xs-x_min)*inv_xw + 0.5f;
      xe = width*(xe-x_min)*inv_xw + 0.5f;
